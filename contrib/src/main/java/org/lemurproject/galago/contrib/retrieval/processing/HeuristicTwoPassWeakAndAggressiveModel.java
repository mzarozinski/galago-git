 ...
 * @author sjh
 */
public class HeuristicTwoPassWeakAndAggressiveModel extends ProcessingModel {

  private LocalRetrieval retrieval;
  private WeakAndAggressiveModel weakAndProcessor;

  public HeuristicTwoPassWeakAndAggressiveModel(LocalRetrieval lr) {
    this.retrieval = lr;
    this.weakAndProcessor = new WeakAndAggressiveModel(lr);
  }
  ...
  public List<ScoredDocument> executeQuery(Node queryTree, Parameters queryParams) throws Exception {
    ...
    System.err.println(new Node("dummy", scoringNodes).toPrettyString());
    System.err.println(new Node("dummy", simpleScoringNodes).toPrettyString());

    
    FixedSizeMinHeap<ScoredDocument> queue = weakAndProcessor.weakAndAlgorithim(simpleScoringNodes, fpRequested, queryParams);
