...
public class CountIndexReader extends KeyListReader implements AggregateIndexPart {
  ...
  public CountIndexReader(BTreeReader reader) throws Exception {
    ...
    stemmer = Stemmer.instance(manifest);
  }
  ...
  public DiskCountIterator getTermCounts(byte[] key) throws IOException {
    ...
    if (iterator != null) {
      return new DiskCountIterator(new CountIndexCountSource(iterator, manifest.getString("compression")));
    }
    return null;
  }
  ...
  public class KeyIterator extends KeyListReader.KeyValueIterator {
    ...
    public String getValueString() {
      ...
      try {
        it = new CountIndexCountSource(iterator, manifest.getString("compression"));
        ns = it.getStatistics();
    ...
    public CountIndexCountSource getStreamValueSource() throws IOException {
      return new CountIndexCountSource(iterator, manifest.getString("compression"));
    }
